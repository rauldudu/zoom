...
function Zoom(elem, config, wnd) {
    ...
    this.zoomHandler = handleTouchEvent(function(touches) {
    ...
    this.touchStartHandler = handleTouchEvent(function(touches) {
        if (touches.length === 1) {
...
Zoom.prototype.activate = function() {
    var target = this.elem && this.elem.parentNode;

    if (target) {
        target.addEventListener('touchstart', this.touchStartHandler);
        target.addEventListener('touchstart', this.zoomHandler);
        target.addEventListener('touchmove', this.zoomHandler);
        target.addEventListener('touchend', this.zoomHandler);
    }
}

Zoom.prototype.deactivate = function() {
    var target = this.elem && this.elem.parentNode;

    if (target) {
        target.removeEventListener('touchstart', this.touchStartHandler);
        target.removeEventListener('touchstart', this.zoomHandler);
        target.removeEventListener('touchmove', this.zoomHandler);
        target.removeEventListener('touchend', this.zoomHandler);
    }
}
